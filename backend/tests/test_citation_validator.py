...
def test_bracketed_years_in_neutral_citations_are_not_markers():
    """'Republic v Mgunya [2010] eKLR' names a case; [2010] is not a reference."""
    from app.scoring import citation_coverage, strip_invalid_citations

    answer = "Bail may be denied for compelling reasons, as in Republic v Mgunya [2010] eKLR [1]."

    cleaned, stripped = strip_invalid_citations(answer, citation_count=1)

    assert cleaned == answer
    assert stripped == []
    assert citation_coverage(answer, citation_count=1).invalid_indices == []
